      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {DICE_FACES.map((face) => (
          <motion.button
            ...
            className={`flex justify-center items-center rounded-lg transition-colors duration-200 aspect-square ${value === face ? 'bg-primary/20 ring-2 ring-primary' : 'bg-accent/50 hover:bg-accent'}`}
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
          >
            <Dice value={face} className="w-10 h-10 sm:w-12 sm:h-12" />
          </motion.button>
        ))}
      </div>
    </div>
      ...
      <DialogContent className="sm:max-w-md bg-card border-border">
        ...
        <div className="space-y-6 py-4 flex flex-col items-center">
            <div className="flex gap-2 sm:gap-4">
              <AnimatePresence>
                <Dice value={d1} className={`w-16 h-16 sm:w-20 sm:h-20 transition-opacity ${d1 === null ? 'opacity-30' : ''}`} />
                <Dice value={d2} className={`w-16 h-16 sm:w-20 sm:h-20 transition-opacity ${d2 === null ? 'opacity-30' : ''}`} />
                <Dice value={d3} className={`w-16 h-16 sm:w-20 sm:h-20 transition-opacity ${d3 === null ? 'opacity-30' : ''}`} />
              </AnimatePresence>
            </div>
        </div>
